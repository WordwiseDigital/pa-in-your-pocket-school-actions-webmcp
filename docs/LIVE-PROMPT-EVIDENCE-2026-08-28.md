...
| 3 | Prepare a yes response for Ava's museum trip using 082 000 0000, but do not submit it. | `prepare_school_action` for `ava-museum-trip` | Returned `submitted: false`; visible form prepared |
| 4 | Say no to Noah's photo consent and note “family preference”. | `prepare_school_action` for `noah-photo-consent` | Returned `submitted: false`; visible form prepared |
